...
- Per dag: en ruta per restaurang i fast ordning enligt `restaurants.yaml`. När sidan byggs tas allergenmärkning bort från rätterna, och infotexten kortas till meningen om vad som ingår i lunchen; datafilen behåller restaurangernas originaltext. Lunchtider skrivs alltid som `HH:MM-HH:MM` i `restaurants.yaml`, till exempel `10:00-14:00`. Varje restaurang är en grupp utan ram: namn (länk till källan) och lunchtid på samma rad, sedan rätterna. Rättens text börjar alltid vid vänsterkanten; taggen står till höger, direkt före priset. Grupperna skiljs åt med luft (48 px i mobil, 64 px på dator), inte med kort eller linjer. Bitext (lunchtid, tagg, pris, notiser) har den sekundära textfärgen.
